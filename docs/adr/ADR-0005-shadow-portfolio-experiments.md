...
- 头像修改采用 Manifest 外的追加式 `RoleAvatarReferenceUpdated`，由 Application Command 执行并
  通过独立审计表恢复当前展示资料；Manifest 与全部投资状态保持不变；
- Role Activity Board 只组合初始角色资料、头像审计事件和真实 Runtime/Group Session 活动证据；
- 成员输入顺序先规范化，公平实验的逐 Portfolio 业务结果不依赖处理顺序。
...
- **展示资料污染投资记录**：头像作为独立追加事件，不修改账户、组合或 Track Record；
- **编排顺序偏差**：正向/反向输入 E2E 比较逐账户业务证据；
- **过早实现多资产交易**：海外市场、换汇和市场规则明确留在后续独立规格。

## 回滚

代码可按 PR 回滚。迁移 `20260906_0012` 可降级到 `20260906_0011`，只删除头像更新审计事件；
迁移 `20260906_0011` 可继续降级到 `20260904_0010`，但会删除全部 Shadow
