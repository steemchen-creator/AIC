...
from aic_backend.domain.market_data import CanonicalRecord, DailyBar, MarketQuote
...
class DataValidationService:
    def __init__(
        ...
        market_quote_validator: Validator[MarketQuote] | None = None,
    ) -> None:
        self._canonical_validator = canonical_validator
        self._daily_bar_validator = daily_bar_validator
        self._market_quote_validator = market_quote_validator
    ...
    def validate(self, record: object) -> ValidationResult:
        ...
        if isinstance(record, MarketQuote) and self._market_quote_validator is not None:
            return self._market_quote_validator.validate(record)
