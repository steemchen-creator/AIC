        ...
        *,
        ...
        result = await self._runtime.execute(
            ProviderRequestContext(
                ...
                timeout_ms,
                symbol=str(
                    parameters.get("symbol", parameters.get("ts_code", ""))
                )
                or None,
            ),
            parameters,
        ...
        succeeded = failed = persisted = already_exists = identity_conflicts = 0
        for row in rows:
            ...
            try:
                ...
                if error.code is not PersistenceErrorCode.IDENTITY_CONFLICT:
                    raise
